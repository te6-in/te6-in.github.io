...
function qs(query) {
	return Array.from(querySelectorAll(query));
}
...
function setRandomColor() {
	...
	const randomColorHSL = "hsl(x,54%,54%)".replace("x", randomH);
	const randomColorHSLA = "hsla(x,54%,54%, 0.2)".replace("x", randomH);
	q("#accent-color-title").style.color = randomColorHSL;
	q("#accent-color-description").style.color = randomColorHSL;
...
function showDarkModeText() {
	const darkModeText = q("#dark-mode");
	if (isDarkMode()) {
...
window.addEventListener("scroll", () => {
	fade();
	hide();
	q("#fixed-viewport-height-1").style.transform = `translate(0px,	${
		-getScrollPercent() * 10
	}em) scale(${1 - getScrollPercent() * 0.3})`;
});
